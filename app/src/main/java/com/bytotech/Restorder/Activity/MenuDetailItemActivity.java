...
public class MenuDetailItemActivity extends AppCompatActivity implements View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cardGoToCart=findViewById(R.id.card_gotocart);
		...
		cardGoToCart.setOnClickListener(new View.OnClickListener() {
			@Override
	...
	public void getGetVideo() {
		...
		Call<BaseVideoResponse> call = apiService.getVideo(strMId);

		//making the call to generate checksum
		call.enqueue(new Callback<BaseVideoResponse>() {
			@Override
			public void onResponse(Call<BaseVideoResponse> call, Response<BaseVideoResponse> response) {
				if (response.body().getResponse().getCode()==200)
				{
					if (response.body().getVedio().size()!=0)
					{
						...
						showVideo(response.body().getVedio());
					}else {
						videoView.setVisibility(View.GONE);
					}
	...
	private void showVideo(ArrayList<String> vedio) {

		Uri uri = Uri.parse("http://phool.spindiabazaar.com/"+vedio.get(0));
