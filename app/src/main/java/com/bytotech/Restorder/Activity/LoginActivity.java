...
import com.bytotech.Restorder.WS.Api;
...
public class LoginActivity extends AppCompatActivity implements View.OnClickListener, GoogleApiClient.OnConnectionFailedListener {
	...
	private EditText edtEmail, edtPassword,edMobile,edOTP;
	private CardView cvEmail, cvPassword, cvMobile,cvOTP;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edMobile=findViewById(R.id.edtMobile);
		edOTP=findViewById(R.id.edtGetOTP);
		cvEmail=findViewById(R.id.cvEmail);
		cvMobile=findViewById(R.id.cvMobile);
			...
			case R.id.ivLogin:
					...
					if (cc.isOnline()) {
						if (from.equals("M"))
						{
							...
							if (Validate.isNull(edMobile.getText().toString())) {
						...
						{
							if (Validate.isNull(edOTP.getText().toString())) {
								edOTP.setError("Please enter OTP.");
								edOTP.requestFocus();
							}else if (!edOTP.getText().toString().trim().equals(otpGet))
							{
	...
	public void getGetOTP() {
		...
		Retrofit retrofit = new Retrofit.Builder().
				...
				baseUrl(Api.BASE_URL).client(httpClient.build()).build();

		//creating the retrofit api service
		Api apiService = retrofit.create(Api.class);

		RequestBody mobile =
				RequestBody.create(MediaType.parse("multipart/form-data"), edMobile.getText().toString());

		//creating a call object from the apiService
		Call<ResponseBaseGetOTP> call = apiService.getLoginOTP(mobile);

		//making the call to generate checksum
		call.enqueue(new Callback<ResponseBaseGetOTP>() {
			@Override
			public void onResponse(Call<ResponseBaseGetOTP> call, Response<ResponseBaseGetOTP> response) {
				progressbar.setVisibility(View.GONE);
			...
			@Override
			public void onFailure(Call<ResponseBaseGetOTP> call, Throwable t) {
				progressbar.setVisibility(View.GONE);
				cc.showToast(getString(R.string.msg_something_went_wrong));
				t.printStackTrace();
			}
